...
clean:
	rm -f $(SRCDIR)/*.o $(TARGET) test_output.bin test_input.txt
...
test: all
	...
	@echo "secretkey123" | ./editor write test_output.bin < test_input.txt
	@echo "Archivo cifrado+comprimido: $$(wc -c < test_output.bin) bytes"
	@echo "secretkey123" | ./editor read test_output.bin > test_recovered.txt
	@diff test_input.txt test_recovered.txt && echo "✓ Integridad verificada: los datos recuperados son idénticos" || echo "✗ ERROR: datos corruptos"
